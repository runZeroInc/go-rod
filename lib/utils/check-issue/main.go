...
func sendComment(id int, msg string) {
	msg += fmt.Sprintf(
		"\n\n_<sub>generated by [check-issue](https://github.com/runZeroInc/go-rod/actions/runs/%s)</sub>_",
		os.Getenv("GITHUB_RUN_ID"),
	)

	q := req(fmt.Sprintf("/repos/runZero/go-rod/issues/%d/comments", id))
...
func deleteComments(id int) {
	...
	for _, c := range list.Arr() {
		if c.Get("user.login").Str() == "github-actions[bot]" &&
			...
			iid := c.Get("id").Int()
			q := req(fmt.Sprintf("/repos/runZero/go-rod/issues/comments/%d", iid))
